...
//		}
		...
		if (Controller.getNodes()==null || Controller.getNodes().size()==0) {
			...
			addNodeToAppButton.setOnAction(addNodeEvent -> {
				...
				if (nodeList.getValue().equalsIgnoreCase("select") || (listofNodesToAddToApp.contains(nodeList.getValue()))) {}
				else {
				...
				if (!listofNodesToAddToApp.isEmpty()) {
					...
					dynamicNodeListLabel.setText(listofNodesToAddToApp.toString().substring(1, listofNodesToAddToApp.toString().length()-1));
				}
			});
